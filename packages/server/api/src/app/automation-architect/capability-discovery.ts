...
export type ActivepiecesActionClassification = 'READ' | 'SEARCH' | 'WRITE' | 'DESTRUCTIVE'
...
export type CapabilitySearchActionResult = {
    pieceName: string
    actionName: string
    displayName: string
...
export type CapabilitySearchTriggerResult = {
    pieceName: string
    triggerName: string
    displayName: string
    oneLineDescription?: string
...
export type CapabilitySearchResponse<T> = {
    results: T[]
    mode: 'semantic' | 'keyword'
    degradeReason?: 'no-embedder' | 'embed-failed'
}
...
type ComponentMetadata = {
    ...
    displayName: string
    description: string
    classification?: ActivepiecesActionClassification
}

export type CapabilityPieceMetadata = {
    name: string
    displayName: string
...
export type ActivepiecesCapabilityDiscoveryDependencies = {
    searchActions(query: string, params: SearchParams): Promise<CapabilitySearchResponse<CapabilitySearchActionResult>>
    searchTriggers(query: string, params: SearchParams): Promise<CapabilitySearchResponse<CapabilitySearchTriggerResult>>
    getPiece(params: {
        name: string
        platformId: string
        projectId: string
    }): Promise<CapabilityPieceMetadata | undefined>
    ...
    async discover(params: ActivepiecesCapabilityDiscoveryParams): Promise<ActivepiecesCapabilityDiscoveryResult> {
        ...
        const [triggerResponse, actionResponse, connectionState] = await Promise.all([
            kinds.includes('TRIGGER')
                ...
                : Promise.resolve<CapabilitySearchResponse<CapabilitySearchTriggerResult> | undefined>(undefined),
            kinds.includes('ACTION')
                ? dependencies.searchActions(query, searchParams)
                : Promise.resolve<CapabilitySearchResponse<CapabilitySearchActionResult> | undefined>(undefined),
        ...
        const pieceCache = new Map<string, Promise<CapabilityPieceMetadata | undefined>>()
...
function resolveActionRisk(classification: ActivepiecesActionClassification | undefined): PlannerCapability['risk'] {
...
function appendDegradeIssue(
    kind: 'action' | 'trigger',
    response: CapabilitySearchResponse<CapabilitySearchActionResult | CapabilitySearchTriggerResult> | undefined,
    issues: ActivepiecesCapabilityDiscoveryIssue[],
): void {
...
async function getPieceCached(params: {
    ...
    pieceCache: Map<string, Promise<CapabilityPieceMetadata | undefined>>
